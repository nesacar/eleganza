...
export default class Siema {
  ...
  addPagination() {
    ...
    for (let i = 0; i < n; i++) {
      ...
      bullet.addEventListener('click', () => this.goTo(i * this.perPage));
      paggination.appendChild(bullet);
    }
    this.selector.parentNode.insertBefore(paggination,
  ...
  _setActiveSlide() {
    const bullets = document.querySelectorAll('.siema-bullet');
    const index = this.currentSlide;
    const bullet = Math.round(index / this.perPage);
    ...
    Array.from(bullets).forEach((child, i) => {
      if (i === bullet) {
